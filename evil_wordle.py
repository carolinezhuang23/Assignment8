...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for index,letter in enumerate(guessed_word):
            if feedback_colors[index] == CORRECT_COLOR:
                self.colors[letter] = CORRECT_COLOR
            elif feedback_colors[index] == WRONG_SPOT_COLOR \
                and self.colors.get(letter) != CORRECT_COLOR:
                self.colors[letter] = WRONG_SPOT_COLOR
            elif feedback_colors[index] == NOT_IN_WORD_COLOR \
                and self.colors.get(letter) != CORRECT_COLOR \
                and self.colors.get(letter) != WRONG_SPOT_COLOR:
    ...
    def __str__(self):
        ...
        """
        row_spaces = [
            (0, self.rows[0]),  # No leading spaces
            (1, self.rows[1]),  # 1 leading space
            (3, self.rows[2])
        ]
        rows = []
        for leading_spaces, row in row_spaces:
            row_str = " " * leading_spaces
            row_str += " ".join(color_word(self.colors[char], char) for char in row)
            rows.append(row_str)
        return "\n".join(rows)
...
def fast_sort(lst):
    ...
    return fast_sort(left_list) + pivot_list + fast_sort(right_list)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if secret_word[i] in dict_word:
            dict_word[secret_word[i]] += 1
        else:
            dict_word[secret_word[i]] = 1
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
            dict_word[guessed_word[i]] -= 1
    for i in range(NUM_LETTERS):
        if guessed_word[i] != secret_word[i]:
            if guessed_word[i] in dict_word and dict_word[guessed_word[i]] > 0:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    word_families = []
    for feedback, word in feedback_groups.items():
        word_families.append(WordFamily(feedback, word))
    hardest_word_family = fast_sort(word_families)[0]

    feedback_colors = hardest_word_family.feedback_colors
    remaining_secret_words = hardest_word_family.words
